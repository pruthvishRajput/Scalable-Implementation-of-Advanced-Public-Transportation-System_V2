...
def GetBoundAndHData(LocationRecord,BusStopsList,VariableDict,RouteName, NpPathDir, UseMongoDB, SingleTripInfo):
    '''
    input: The location record update, list of bus-stops on a route, variable dict, and route name, the path of stored NpData, and flag to indicate whether the MongoDB data or NP data is used.
    ...
    SingleTripInfoSplitted = SingleTripInfo.split('_')
    TripStartTime = SingleTripInfoSplitted[4]
    ...
    if UseMongoDB==True:
        ...
        HistoricalDataList = [Record for Record in con[RouteName]['H.'+TripStartTime+'.'+VariableDict['Bound']+'.'+SingleTripInfo].find().sort([('id',1)])]
    else:
        Bound_Temp = VariableDict['Bound']
        HistoricalDataList = np.load(f'{NpPathDir}/{RouteName}/H.{TripStartTime}.{Bound_Temp}.{SingleTripInfo}.npy', allow_pickle=True)
...
def RecordPredictionNorthBound (LocationRecord,VariableDict):
    ...
    if VariableDict['t_j_p1_predicted'] != -1:
        ...
        PredictionDict['PredictionAvailable'] = True
        ...
        PredictionDict['PredictionAvailable'] = False
