...
def ConvertToEarthaxisAcc(RouteName,RecordType):
	'''
	...
	for index in range(len(SingleTripsInfo)):
		...
		StoppageSegmentsList = StoppageSegments[0]['StopSegmentsGPSIndex']
		
		#StopSegmentsGPSIndex=[]
		for indexj in range(len(StoppageSegmentsList)):
			AcclMagRecordsListStoppages =[AclR for AclR in con[RouteName][SingleTripsInfo[index]+".AcclMagData.Segment"].find({ '$and': [ { 'GPSIndex': { '$gte': StoppageSegmentsList[indexj][0]} }, { 'GPSIndex': {'$lte':StoppageSegmentsList[indexj][1]} } ] }).sort([('GPSIndex',1)])]
			#GetLinearAndGravityAcc (AcclMagRecordsListStoppages)
			#ProcessEarthaxisAcceleration (RouteName,SingleTripsInfo[index],AcclMagRecordsListStoppages)
			ProcessEarthaxisHVComponentUsingJigSawMethod (RouteName,SingleTripsInfo[index],AcclMagRecordsListStoppages,RecordType)
			#input()
		con[RouteName]['TripsInfo'].update_one({'SingleTripInfo':SingleTripsInfo[index]},{'$set':{'ConvertedToEarthAxis':True}})
