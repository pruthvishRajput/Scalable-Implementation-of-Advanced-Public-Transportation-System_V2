...
def PlotPrediction(SingleTripInfo,RouteName, ResultPathDir, NpPathDir, ResultPathDir_Np, UseMongoDB):
	'''
	input: The trip selected for arrival time prediction, data directories path, and flag to indicate whether the MongoDB data or NP data is used.
	...
	if UseMongoDB==True:
		...
		PredictionDictList = np.load(f'{ResultPathDir_Np}{RouteName}/{SingleTripInfo}.PredictionResult_Dist_th_50.npy', allow_pickle=True)
	...
	filename=SingleTripInfo+".png"
	#pl.savefig('test.eps', format='eps', dpi=900)	
	plt.savefig(f'{ResultPathDir}Prediction_{filename}', format='png', dpi=600)
	plt.show()
	#print(ax1.get_xlim())
	#print(ax1.get_ylim())
	#input()
