...
def ComputeFeatureForComponents(RouteName,AcclRecords,ModeInt,Window,SingleTripInfo):
    ...
    '''
    ComputeFeature (RouteName,AcclH, AcclV, ModeInt, Window, SingleTripInfo)#,ComponentType)
...
def ApplyWindow(AcclH, AcclV, Window):
	...
	AcclH = trim_mean(AcclH,0.4,axis=1)
	AcclV = trim_mean(AcclV,0.4,axis=1)
...
def ComputeFeature(BodyAcclX,BodyAcclY,BodyAcclZ,
                    ...
                    ModeInt,Window,SingleTripInfo):#,ComponentType):
'''
def ComputeFeature (RouteName,AcclH, AcclV, ModeInt, Window, SingleTripInfo):#,ComponentType) 
    SamplingFreq = 40
    TimeBetweenSamples = 1/40
    index = 0
    ...
    for (WAcclH , WAcclV) in zip (AcclH , AcclV):
        ...
        con[RouteName][SingleTripInfo+'.HARFirstVersionFeaturesExtraction.TrimMean'].insert_one(FeaturesDict)
        index+=1
