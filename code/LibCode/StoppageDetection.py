...
def MarkDetectedBusStopOnAMap(LocationTupleList, model, clusters,LocationRecord, map_osm, ResultPathDir):
	'''
	input: Location records (in tuple form) of the trip, DBSCAN model object, clusters identified 
		   using the DBSCAN algorithm, map object, and result directory path to store the locations of detected stoppages.
	...
	File = open(f'{ResultPathDir}/BusStopLocations.txt','w')
	...
	for index in range(0 , clusters):
		...
		File.write(f'Location of bus-stop: Latitue: {busStopLat}, Longitude: {busStopLon} \n')
