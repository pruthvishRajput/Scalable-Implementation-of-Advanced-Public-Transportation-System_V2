...
def HistoricalWeights(SingleTripDelInfo,RouteName):
	...
	try:
		...
		if '__18' in SingleTripDelInfoRecord[0]['SingleTripInfo']: 
			DelBound = 'South'
			DelStartHour = '18'
		else:
			DelBound = 'North'
...
def IterateThroughAggregrateTrips(SingleTripDelInfo, DelBound, DelStartHour, TripStartTimeAggregate, BusStopsList, BusStopsCount, Bound, RouteName):
	...
	for StartTimeBoundIndex in range(0,len(TripStartTimeAggregate[0])):
		...
		CreateHistoricalDictObjectAndStoreInMongo(T_pt_list, F_ps_list, TripStartTimeAggregate, StartTimeBoundIndex, RouteName, SingleTripDelInfo)
...
def CreateHistoricalDictObjectAndStoreInMongo(T_pt_list, F_ps_list, TripStartTimeAggregate, StartTimeBoundIndex, RouteName, SingleTripDelInfo):
	'''
	input: The list of temporal and spatial travel time estimates for every pair of bus-stops, trip start time, bound, route name, selected trip name which is to be excluded.
	output: void
	...
	con[RouteName].drop_collection('H.'+str(TripStartTimeAggregate[0][StartTimeBoundIndex][0])+'.'+str(TripStartTimeAggregate[0][StartTimeBoundIndex][1])+'.'+SingleTripDelInfo)
	con[RouteName]['H.'+str(TripStartTimeAggregate[0][StartTimeBoundIndex][0])+'.'+str(TripStartTimeAggregate[0][StartTimeBoundIndex][1])+'.'+SingleTripDelInfo].insert_many(DiffMeanStd)
