...
def GetBMAndPDSegments(RouteName,BMSingleTripsInfoList,StopSpeed,UpperSpeedLimit):
	'''
	input: The route name, trips list, range of speeds to extract the stoppage segments.
	output: None
	...
	for index in range(len(BMSingleTripsInfoList)):
		...
		LocationRecordsListStoppages =[LR for LR in con[RouteName][BMSingleTripsInfoList[index]+".GPSRecord.Raw"].find({ '$and': [ { 'Speed': { '$gte': 0 } }, { 'Speed': {'$lte':UpperSpeedLimit} } ] }).sort([('Time',1)])]
		StartSegment = False
		StoreSegment =False

		for indexj in range(len(LocationRecordsListStoppages)-1):
	    	#if LocationRecordsListStoppages[indexj]['Speed']==0.0 and LocationRecordsListStoppages[indexj+1]['Speed']>0.0:
			if StartSegment== False  and LocationRecordsListStoppages[indexj]['Speed']<StopSpeed:
				...
				#print(LocationRecordsListStoppages[indexj]["Time"],LocationRecordsListStoppages[indexj]["Latitude"],LocationRecordsListStoppages[indexj]["Longitude"],LocationRecordsListStoppages[indexj]["Accuracy"],LocationRecordsListStoppages[indexj]["Speed"])
...
def GetCMSegments(RouteName,CMSingleTripsInfoList,BMSingleTripsInfoList):
	'''
	input: The route name, trips list of Commuter Module (CM) and Bus Module (BM) devices, range of speeds to extract the stoppage segments.
	output: None
	function: It extracts the stoppage segments by mapping the timestamps of the BM and CM devices and using the GPS records of BM device due to low accuracy of CM dvice. The extracted segments are stored it in MongoDB.
	'''
	...
	for index in range(len(CMSingleTripsInfoList)):
		...
		StoppageSegments = [Segment for Segment in con[RouteName]['TripsInfo'].find({'SingleTripInfo':"BM."+dateTimeOP})]
